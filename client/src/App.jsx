...
const programID = new PublicKey("7DJJzXPcZMLcxNG3k84MqzzNqZenn92NEPanNEr3H6RN");
  ...
  const connectWallet = async () => {
    if(window.solana){
      try {
        ...
        const walletAddresss = window.solana.publicKey.toString();
        setWalletAddress(walletAddresss);
        setError(null);
      } catch (error) {
        setError("Phantum wallet connection failed")
      } finally {
        setLoading(false)
      }
    } else {
      console.error("Please install Phantom Wallet")
      setError("Phantum wallet not found")
    }
  };
