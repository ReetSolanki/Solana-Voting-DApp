...
mod contexts;
mod state;
use contexts::*; // To include all pub structs and functions with sirect name
mod errors;
use errors::*;
mod events;
use events::*;
declare_id!("7DJJzXPcZMLcxNG3k84MqzzNqZenn92NEPanNEr3H6RN");
use anchor_lang::system_program;
use anchor_spl::token::{mint_to, transfer, MintTo, Transfer};

#[program]
pub mod vote_app {
    //use crate::instruction::BuyTokens;
    ...
    pub fn pick_winner(ctx: Context<PickWinner>, proposal_id: u8) -> Result<()> {
        ...
        //Checking if proposals has any votes
        require!(proposal.number_of_votes > 0, VoteError::NoVotesCast);

        // Update Winner if this proposal has more votes (or if no winner yet)
